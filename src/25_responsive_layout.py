...
Meaning that the layout reacts to the change of the window, going from a list of widgets to
columns of the same widgets.

For this to work different layouts are needed. The problem is that Tkinter does not have the
built-in tools for responsive layouts.

That is why we can not update an existing layout but need to create seperate layouts for
differnet window sizes.

A way to make this work is to find out the size of the window when it resizes and having different
methods to do the same thing but for different window sizes.
...
class App(tk.Tk):
    ...
    def create_medium_layout(self):
        ...
        ttk.Label(self.frame, text="Label 1", background="red").grid(column=0, row=0, sticky="nswe", padx=20, pady=10)
        ttk.Label(self.frame, text="Label 2", background="green").grid(column=1, row=0, sticky="nswe", padx=20, pady=10)
        ttk.Label(self.frame, text="Label 3", background="blue").grid(column=0, row=1, sticky="nswe", padx=20, pady=10)
        ttk.Label(self.frame, text="Label 4", background="yellow").grid(column=1, row=1, sticky="nswe", padx=20, pady=10)
    ...
    def create_large_layout(self):
        ...
        ttk.Label(self.frame, text="Label 1", background="red").grid(column=0, row=0, sticky="nswe", padx=20, pady=10)
        ttk.Label(self.frame, text="Label 2", background="green").grid(column=1, row=0, sticky="nswe", padx=20, pady=10)
        ttk.Label(self.frame, text="Label 3", background="blue").grid(column=2, row=0, sticky="nswe", padx=20, pady=10)
        ttk.Label(self.frame, text="Label 4", background="yellow").grid(column=3, row=0, sticky="nswe", padx=20, pady=10)
